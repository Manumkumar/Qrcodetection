...
def check_cuda_support():
    ...
    try:
        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
            logging.info(f"CUDA devices available: {cv2.cuda.getCudaEnabledDeviceCount()}")
            # Test basic CUDA operation
            test_mat = cv2.cuda_GpuMat()
            test_cpu = np.zeros((100, 100), dtype=np.uint8)
            test_mat.upload(test_cpu)
            test_result = test_mat.download()
            logging.info("✅ CUDA support verified")
            return True
        else:
...
def digital_zoom_cuda(frame_gpu, zoom_factor, center=None):
    """CUDA-accelerated digital zoom"""
    if zoom_factor <= 1.0:
        return frame_gpu
    
    # Download to get dimensions (this is fast)
    h, w = frame_gpu.size()[::-1]  # GpuMat returns (width, height)
    ...
    # Adjust if crop area is too small
    if x2 - x1 < new_w:
        x1 = max(0, w - new_w)
        x2 = w
    if y2 - y1 < new_h:
    ...
    # Create ROI and resize on GPU
    roi = cv2.Rect(x1, y1, x2 - x1, y2 - y1)
    cropped_gpu = cv2.cuda_GpuMat(frame_gpu, roi)
    
    # Resize on GPU
    zoomed_gpu = cv2.cuda_GpuMat()
    cv2.cuda.resize(cropped_gpu, zoomed_gpu, (w, h), interpolation=cv2.INTER_LINEAR)
    
    return zoomed_gpu

def digital_zoom_cpu(frame, zoom_factor, center=None):
    """CPU fallback for digital zoom"""
    if zoom_factor <= 1.0:
        return frame
    h, w = frame.shape[:2]
    if center is None:
...
def preprocess_image_fast_cuda(frame_gpu):
    """CUDA-accelerated fast preprocessing"""
    gray_gpu = cv2.cuda_GpuMat()
    if frame_gpu.channels() == 3:
        cv2.cuda.cvtColor(frame_gpu, gray_gpu, cv2.COLOR_BGR2GRAY)
    else:
        gray_gpu = frame_gpu
    return gray_gpu

def preprocess_image_thorough_cuda(frame_gpu):
    """CUDA-accelerated thorough preprocessing"""
    gray_gpu = cv2.cuda_GpuMat()
    if frame_gpu.channels() == 3:
        cv2.cuda.cvtColor(frame_gpu, gray_gpu, cv2.COLOR_BGR2GRAY)
    else:
    ...
    equalized_gpu = cv2.cuda_GpuMat()
    cv2.cuda.equalizeHist(gray_gpu, equalized_gpu)
    
    # Adaptive threshold on GPU
    thresh_gpu = cv2.cuda_GpuMat()
    cv2.cuda.threshold(equalized_gpu, thresh_gpu, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    
    # Download for sharpening (morphology operations are limited on GPU)
    ...
    kernel = np.array([[-1, -1, -1],
                       [-1,  9, -1],
                       [-1, -1, -1]])
    sharp = cv2.filter2D(thresh_cpu, -1, kernel)
    
    return sharp

def preprocess_image_fast_cpu(frame):
    """CPU fallback for fast preprocessing"""
...
def preprocess_image_thorough_cpu(frame):
    """CPU fallback for thorough preprocessing"""
    if len(frame.shape) == 3:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    else:
        gray = frame
...
class QRDetector:
    def __init__(self, use_cuda=False):
        ...
        # Initialize GPU memory pools if using CUDA
        if self.use_cuda:
            try:
                cv2.cuda.setDevice(CUDA_DEVICE_ID)
                self.temp_gpu = cv2.cuda_GpuMat()
                self.gray_gpu = cv2.cuda_GpuMat()
                logging.info("✅ CUDA GPU memory initialized")
            except Exception as e:
    ...
    def detect_qr_codes(self, frame, thorough=False):
        with self.suppress_warnings():
            if self.use_cuda and isinstance(frame, cv2.cuda_GpuMat):
                # GPU processing
                if thorough:
                    processed = preprocess_image_thorough_cuda(frame)
                else:
                    processed_gpu = preprocess_image_fast_cuda(frame)
                ...
                if thorough:
                    processed = preprocess_image_thorough_cpu(frame)
                else:
                    processed = preprocess_image_fast_cpu(frame)
            ...
            if not codes and thorough:
                ...
                if isinstance(frame, cv2.cuda_GpuMat):
                    ...
                    frame_cpu = frame
                codes = pyzbar.decode(frame_cpu, symbols=[pyzbar.ZBarSymbol.QRCODE])
...
def initialize_camera():
    """Initialize camera with GStreamer pipeline for Jetson Nano"""
    # Try GStreamer pipeline first (optimal for Jetson)
    gstreamer_pipeline = (
        f"nvarguscamerasrc sensor-id=0 ! "
        f"video/x-raw(memory:NVMM), width={FRAME_WIDTH}, height={FRAME_HEIGHT}, format=NV12, framerate=30/1 ! "
    ...
    if not cap.isOpened():
        logging.warning("GStreamer pipeline failed, trying USB camera...")
...
def main():
    # Check CUDA support
    cuda_available = check_cuda_support() if USE_CUDA else False
    
    detector = QRDetector(use_cuda=cuda_available)
    ...
    window_name = f"{CAMERA_NAME} - QR Scanner ({'CUDA' if cuda_available else 'CPU'})"
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(window_name, display_width, display_height)
    
    # GPU memory for CUDA processing
    if cuda_available:
        frame_gpu = cv2.cuda_GpuMat()
        frame_zoomed_gpu = cv2.cuda_GpuMat()
    ...
    try:
        while True:
            ...
            # Upload frame to GPU if using CUDA
            if cuda_available:
                frame_gpu.upload(frame)
                working_frame = frame_gpu
            else:
                working_frame = frame
            ...
            if should_process:
                ...
                codes = detector.detect_qr_codes(working_frame, thorough=False)
            ...
            # Apply zoom
            if cuda_available:
                frame_zoomed_gpu = digital_zoom_cuda(working_frame, zoom_factor, center=zoom_center)
                frame_zoomed = frame_zoomed_gpu.download()
            else:
                frame_zoomed = digital_zoom_cpu(working_frame, zoom_factor, center=zoom_center)
            
            # Detect QR codes in zoomed frame
            if cuda_available:
                codes_zoomed = detector.detect_qr_codes(frame_zoomed_gpu, thorough=should_process)
            else:
            ...
            mode_text = "CUDA" if cuda_available else "CPU"
            cv2.putText(frame_zoomed, f"Mode: {mode_text}", (10, 200),
